...
const ModernScoringPage = () => {
  const { matchId, leagueId } = useParams<{ matchId: string; leagueId: string }>()
  ...
  useEffect(() => {
    if (match) {
      const games1 = [match.team1_game1 || 0, match.team1_game2 || 0, match.team1_game3 || 0]
      const games2 = [match.team2_game1 || 0, match.team2_game2 || 0, match.team2_game3 || 0]
  ...
  const calculateSets = (games1: number[], games2: number[]) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      else if (games2[i] > games1[i]) sets2++
    }
    return { sets1, sets2 }
  }
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <Link to={`/league/${leagueId}/matches`} className="text-blue-400 hover:text-blue-300">
            Back to matches
          </Link>
        </div>
      </div>
  ...
  const team1Name = match.match_type === 'singles'
    ? getPlayerName(match.team1_player1_name)
    : `${getPlayerName(match.team1_player1_name)} / ${getPlayerName(match.team1_player2_name)}`

  const team2Name = match.match_type === 'singles'
    ? getPlayerName(match.team2_player1_name)
    : `${getPlayerName(match.team2_player1_name)} / ${getPlayerName(match.team2_player2_name)}`

  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between p-4">
        <Link to={`/league/${leagueId}/matches`} className="p-2 hover:bg-gray-800 rounded-lg">
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-8">
        ...
        <div className="flex justify-between items-center mb-12">
          <h2 className="text-2xl font-light text-left max-w-[40%]">
            {team1Name}
          </h2>
          <h2 className="text-2xl font-light text-right max-w-[40%]">
            {team2Name}
          </h2>
        </div>
            ...
            {[0, 1, 2].map((gameIndex) => (
              <div key={gameIndex} className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gray-800 rounded border border-gray-600 flex items-center justify-center">
                  <span className="text-lg font-medium">{team1Games[gameIndex]}</span>
                </div>
                ...
                <div className="w-12 h-12 bg-gray-800 rounded border border-gray-600 flex items-center justify-center">
                  <span className="text-lg font-medium">{team2Games[gameIndex]}</span>
                </div>
              </div>
            ))}
